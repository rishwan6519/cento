...
import { XCircle, Clock, Play, Calendar, Mic, Upload } from "lucide-react";
...
interface AnnouncementItem {
  file: string; // This will hold the file's _id
  displayOrder: number;
  delay: number;
}
...
const AnnouncementCreator: React.FC<AnnouncementCreatorProps> = ({ onCancel, onSuccess }) => {
  ...
  const handleFileToggle = (file: AnnouncementFile) => {
    ...
    if (isSelected) {
      ...
      const newItem: AnnouncementItem = {
        file: file._id, // Use the file's ID, not its path!
  ...
  const validatePlaylist = (): string | null => {
    ...
    if (scheduleType === 'hourly') {
      if (!startDate) return "Please set start date for frequency based schedule";
      if (!endDate) return "Please set end date for frequency based schedule";
      if (!startTime) return "Please set start time for frequency based schedule";
      if (!endTime) return "Please set end time for frequency based schedule";
      if (!frequency || frequency < 1) return "Please set a valid frequency for the hourly schedule";
    }
    if (scheduleType === 'timed') {
      if (!startDate) return "Please set start date for timed schedule";
      if (!endDate) return "Please set end date for timed schedule";
      if (!startTime) return "Please set start time for timed schedule";
      if (!endTime) return "Please set end time for timed schedule";
    }
    return null;
  };
  ...
  // This function now works correctly because playlist.announcements stores _id
  ...
  // (Rendering logic for isSaved, the main form, etc.)
  // ...
  if (isSaved) {
    return (
      <div className="bg-white rounded-xl shadow-sm p-6 text-black text-center">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">
            Announcement playlist "{savedPlaylistName}" has been saved successfully.
          </p>
        </div>
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 text-black">
      <div className="mb-6 border-b pb-4">
        ...
        <p className="text-gray-600 text-sm mt-1">
          Schedule and organize your announcements
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                { type: 'hourly' as const, label: 'Frequency Based', desc: 'Play at regular intervals', icon: Clock },
                { type: 'timed' as const, label: 'Scheduled Time', desc: 'Play at specific times', icon: Calendar }
              ].map(({ type, label, desc, icon: Icon }) => (
                <div
                  key={type}
                  onClick={() => handleScheduleTypeChange(type)}
                  className={`p-3 border rounded-lg cursor-pointer transition-all ${
                    playlist.schedule.scheduleType === type
          ...
          <div className="space-y-4 ">
              ...
              {playlist.schedule.scheduleType === 'hourly' && (
                <div>
                  <label className="block text-sm font-medium mb-2">Frequency (min)</label>
                  <input
                    type="number"
                    min="1"
                    ...
                    onChange={(e) => setPlaylist(prev => ({
                      ...
                      schedule: {
                        ...prev.schedule,
                        frequency: parseInt(e.target.value, 10) || 1
                      }
                    }))}
                  ...
                  {daysList.map((day) => (
                    <label
                      ...
                      className={`flex items-center gap-1 px-3 py-2 rounded-lg border cursor-pointer transition-all ${
                        playlist.schedule.daysOfWeek?.includes(day.value)
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={playlist.schedule.daysOfWeek?.includes(day.value) || false}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium">
                Select Announcement Files
              </label>
              <button
                onClick={refreshAnnouncementFiles}
            ...
            <div className="max-h-80 overflow-y-auto border rounded-lg p-3">
              {isLoadingFiles ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500">Loading announcement files...</p>
                </div>
              ) : error ? (
                <div className="text-center py-8">
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">No announcement files available</p>
                  <p className="text-xs mt-1">Create some announcements first</p>
                </div>
              ) : (
                announcementFiles.map((file) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={playlist.announcements.some(a => a.file === file._id)}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <span className={`px-2 py-1 rounded ${
                          file.type === 'generated' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'
                        }`}>
                          {file.type}
                        </span>
                        {file.voice && <span>Voice: {file.voice}</span>}
                      </div>
                    </div>
                  </div>
                ))
                ...
                playlist.announcements.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <input
                            ...
                            max="300"
                            value={item.delay}
                            onChange={(e) => updateAnnouncementDelay(
                              item.file, 
                      ...
                      <button
                        onClick={() => removeAnnouncement(item.file)}
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <button
            ...
            className="px-6 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg font-medium"
          >
            {isLoading ? (
              <div className="flex items-center gap-2">
