...
const PlaylistSetup: React.FC<ConnectPlaylistProps> = ({
  ...
  const [
   ...
   useEffect(() => {
      const userRole = localStorage.getItem("userRole");
      if (userRole !== "user" && userRole !== "superUser" && userRole !== "reseller" && userRole !== "store") {
        toast.error("You are not authorized to access this page.");
        window.location.href = "/login";
        return;
      }
 ...
 const openBgImageSelector = (audioPath: string) => {
  ...
  useEffect(() => {
     ...
     const fetchDevices = async () => {
       setIsLoading(true);
       try {
         const res = await fetch(`/api/onboarded-devices?userId=${userIds}`);
     ...
     const fetchPlaylists = async () => {
       setIsLoading(true);
       try {
         const res = await fetch(`/api/playlists?userId=${userIds}`);
