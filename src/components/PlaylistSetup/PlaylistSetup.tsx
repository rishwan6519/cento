...
export default function PlaylistSetup() {
  ...
  const handleSavePlaylistConfig = async () => {
    if (
      ...
      toast.error("Please add a name, at least one file, date range, and select at least one day for the playlist");
      return;
    }
    ...
    try {
      const configToSend = {
        name: playlistConfig.name,
        type: playlistConfig.type || "mixed",
        startTime: playlistConfig.startTime || "00:00:00",
        endTime: playlistConfig.endTime || "00:10:00",
        startDate: playlistConfig.startDate,
        endDate: playlistConfig.endDate,
      ...
      const formData = new FormData();
      formData.append("config", JSON.stringify(configToSend));
      const res = await fetch(`/api/playlist-config?userId=${userId}`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
...
<br/>
            ...
            <div className="flex-1 overflow-y-auto max-h-[420px] pr-2">
              {loadingMedia ? (
                ...
                //           <input type="checkbox" checked={checked} onChange={() => toggleSelect(m)} />
