...
import Card from '../Platform/Card';
import Button from '../Platform/Button';
import { MonitorSmartphone, User, X } from 'lucide-react';
import Image from 'next/image';
import { FaCheckCircle } from 'react-icons/fa';
import { motion, AnimatePresence } from "framer-motion";
...
export default function AssignDevice() {
 ...
 const handleDisconnectDevice = async () => {
    try {
      ...
      // Find the selected device to get the assignment ID
      ...
      const response = await fetch(`/api/assign-device?userId=${assignmentId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const renderStep = () => {
    switch (step) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="flex gap-4 mt-4">
              <Button
                variant={actionType === 'assign' ? 'primary' : 'secondary'}
                onClick={() => setActionType('assign')}
              >
                Assign Device
              </Button>
              <Button
                variant={actionType === 'disconnect' ? 'primary' : 'secondary'}
                onClick={() => setActionType('disconnect')}
              >
                Disconnect Device
              </Button>
            </div>
        ...
        return (
          <div className="space-y-6">
            <h3 className="text-xl font-semibold text-gray-800">
              Step 2: Select Device ({actionType === 'assign' ? 'Assign' : 'Disconnect'})
            </h3>
                ...
                connectedDevices.map((device) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Serial: {device.deviceId.serialNumber}</p>
                      <p className="text-sm text-gray-500">Color: {device.deviceId.color}</p>
                      <p className="text-sm text-gray-500">Assigned: {new Date(device.assignedAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))
              )}
            ...
            {/* Show message if no devices available */}
            {actionType === 'assign' && devices.length === 0 && (
              <div className="text-center py-8">
                <MonitorSmartphone className="mx-auto h-12 w-12 text-gray-400 mb-4" />
                <p className="text-gray-500">No available devices found</p>
              </div>
            ...
            {/* Show message if no connected devices */}
            {actionType === 'disconnect' && connectedDevices.length === 0 && (
              <div className="text-center py-8">
                <MonitorSmartphone className="mx-auto h-12 w-12 text-gray-400 mb-4" />
      ...
      case 3:
        return (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold">Step 3: Confirm {actionType === 'assign' ? 'Assignment' : 'Disconnection'}</h3>
            <div className="p-4 border rounded-lg bg-gray-50">
              <p className="mb-2">
                <span className="font-medium">Selected User: </span>
                {users.find(u => u._id === selectedUser)?.username}
              ...
              <p className="mb-2">
                ...
                {actionType === 'assign'
                  ? devices.find(d => d._id === selectedDevice)?.deviceId.name
                  : connectedDevices.find(d => d._id === selectedDevice)?.deviceId.name}
              </p>
              <p className="mb-2">
                <span className="font-medium">Serial Number: </span>
                {actionType === 'assign'
                  ? devices.find(d => d._id === selectedDevice)?.deviceId.serialNumber
                  : connectedDevices.find(d => d._id === selectedDevice)?.deviceId.serialNumber}
              </p>
              <p>
                <span className="font-medium">Action: </span>
                {actionType === 'assign' ? 'Assign Device' : 'Disconnect Device'}
              </p>
            </div>
            <div className="flex gap-4">
              <Button variant="secondary" onClick={() => setStep(2)}>
              ...
              <Button
                ...
                disabled={loading}
              >
                {loading ? (actionType === 'assign' ? 'Assigning...' : 'Disconnecting...') : (actionType === 'assign' ? 'Confirm Assignment' : 'Confirm Disconnection')}
              </Button>
            </div>
          </div>
        );
  ...
  return (
    <Card className="max-w-2xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">Assign / Disconnect Device</h2>
          <div className="flex items-center gap-2">
            {[1, 2, 3].map((stepNumber) => (
              <div
                key={stepNumber}
        ...
        {showSuccess && (
          ...
          >
            ...
            <motion.h2
              ...
              className="text-3xl font-bold text-green-700 mb-2"
            >
              {actionType === 'assign' ? 'Device Assigned Successfully!' : 'Device Disconnected Successfully!'}
            </motion.h2>
            <p className="text-lg text-gray-700">Redirecting to platform...</p>
          </motion.div>
        )}
