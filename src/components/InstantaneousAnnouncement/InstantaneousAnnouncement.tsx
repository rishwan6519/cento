...
const InstantaneousAnnouncement: React.FC<InstantaneousAnnouncementProps> = ({ onCancel, onSuccess }) => {
  ...
  const createWavBlob = (pcmData: ArrayBuffer): Blob => {
    ...
    const handleGenerateTts = async () => {
        ...
        try {
            const response = await fetch('/api/tts/generate-tts', {
                method: 'POST',
               ...
               return (
                ...
                >
                    <div className="flex items-center gap-3">
                      <img src={device.imageUrl || "/placeholder.jpg"} alt={device.name} className="w-12 h-12 rounded-full object-contain" />
                      <div>
