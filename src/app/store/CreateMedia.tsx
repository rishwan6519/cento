...
import { Upload, Trash2, Clock, CheckCircle2, AlertCircle } from "lucide-react";
...
const CreateMedia: React.FC<CreateMediaProps> = ({ onCancel, onSuccess }) => {
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      const response = await fetch("/api/media-groups", {
        ...
        body: JSON.stringify({ userId, name: newGroupName.trim(), description: "", mediaIds: [] }),
      });
      if (response.ok) {
        const data = await response.json();
    ...
    setFiles((prev) =>
      prev.map((f) => {
        if (f.id === id && !f.isVerified) {
          if (currentTime >= MIN_PLAY_TIME) return { ...f, isVerified: true, listenedTime: MIN_PLAY_TIME };
  ...
  const handleAudioError = (id: string) => {
    setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, isCorrupted: true } : f)));
  };

  const allVerified = files.length > 0 && files.every((f) => f.isVerified);
  const hasCorruptedFiles = files.some((f) => f.isCorrupted);

  const uploadFile = (fileObj: SelectedFile, uid: string): Promise<string | null> => {
  ...
  const handleSubmit = async () => {
    if (!allVerified || hasCorruptedFiles || !userId) return;
    setIsLoading(true);
  ...
  return (
    <div className="bg-[#DFF4F7] min-h-screen flex justify-center items-start p-6">
      <div className="bg-white rounded-2xl shadow-md w-full max-w-7xl p-6">
        <h2 className="text-lg font-semibold mb-6 flex justify-between">
          ...
          <span className="text-sm font-normal text-slate-500">{files.length} / {MAX_FILES} files</span>
        </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div
              className={`border-2 border-dashed rounded-lg h-56 flex flex-col items-center justify-center cursor-pointer transition ${isDragging ? "border-teal-500 bg-teal-50" : "border-gray-300"}`}
              onClick={() => document.getElementById("store-media-upload")?.click()}
            ...
            >
              ...
              <input id="store-media-upload" type="file" multiple hidden accept="audio/*" onChange={(e) => e.target.files && processFiles(Array.from(e.target.files))} />
            </div>
                  ...
                  {loadingGroups ? (
                    ...
                    <>
                      <div className="max-h-48 overflow-y-auto space-y-2">
                        {groups.map((group) => (
                          <div key={group._id} onClick={() => setSelectedGroupId(group._id)} className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition ${selectedGroupId === group._id ? "border-orange-500 bg-orange-50" : "border-slate-200 bg-white"}`}>
                        ...
                        {groups.length === 0 && <p className="text-sm text-gray-400 text-center py-2">No groups yet.</p>}
                      </div>
                      {!showCreateGroup ? (
                  ...
                  <div key={file.id} className={`rounded-lg p-4 border shadow-sm transition-colors ${file.isCorrupted ? "bg-red-50 border-red-200" : "bg-[#EAF8FC] border-blue-100"}`}>
                    {file.isCorrupted && (
                      <div className="flex items-center gap-2 text-red-600 text-xs font-bold mb-3 p-2 bg-red-100 rounded-md">
                        <AlertCircle size={14} />
                        <span>File is corrupted or unplayable. Please remove it.</span>
                      </div>
                    )}
                    ...
                    <audio src={file.previewUrl} controls className="w-full h-8" onTimeUpdate={(e) => handleTimeUpdate(file.id, e.currentTarget.currentTime)} onError={() => handleAudioError(file.id)} />
                    <div className="w-full h-1.5 bg-slate-200 rounded-full mt-3 overflow-hidden">
                      <div className={`h-full transition-all duration-300 ${file.isCorrupted ? "bg-red-400" : file.status === "uploading" ? "bg-blue-500" : file.isVerified ? "bg-green-500" : "bg-orange-400"}`}
        ...
        <div className="mt-10 border-t pt-6 flex flex-col items-center gap-4">
          {hasCorruptedFiles ? (
            <p className="text-sm text-red-600 font-bold flex items-center gap-2 animate-pulse">
              <AlertCircle size={16} /> One or more files are corrupted. Please remove them.
            </p>
          ) : !allVerified && files.length > 0 && (
            <p className="text-sm text-red-500 font-medium animate-pulse">
              * Play all files for at least 5 seconds to unlock upload
            </p>
          )}
          <div className="flex gap-4">
            <button onClick={onCancel} disabled={isLoading} className="px-8 py-2 border rounded-lg hover:bg-slate-50 disabled:opacity-50">Cancel</button>
            <button
              onClick={handleSubmit}
              disabled={isLoading || !allVerified || hasCorruptedFiles || (assignToGroup && !selectedGroupId)}
              className={`px-10 py-2 rounded-lg text-white font-semibold transition-all shadow-md ${allVerified && !hasCorruptedFiles && !isLoading && (!assignToGroup || selectedGroupId) ? "bg-teal-700 hover:bg-teal-800" : "bg-slate-400 cursor-not-allowed"}`}
            >
