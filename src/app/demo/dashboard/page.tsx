...
// ─── Types ───────────────────────────────────────────────────────────────────
// Re-export the same ViewKey type for reused store_user components
export type ViewKey =
  | "dashboard"
  | "mediaManagement"
  | "viewAllCampaigns"
...
const DemoSidebar: React.FC<SidebarProps> = ({
  activeView,
  onNavigate,
  mobileOpen,
  setMobileOpen,
    ...
    <>
      ...
      <aside className={`store-sidebar ${mobileOpen ? "store-sidebar--open" : ""}`}>
        ...
        <div className="store-sidebar__logo">
          <div className="store-logo-icon" style={{ background: "linear-gradient(135deg, #F05A28, #E94622)" }}>
            <MdComputer size={20} />
          </div>
          <div>
            <p className="store-logo-title">DeviceHub</p>
            <p className="store-logo-subtitle">
              demo portal
              <span style={{
                background: "rgba(17,181,187,0.2)",
                color: "#11B5BB",
                fontSize: "0.6rem",
                fontWeight: 700,
                padding: "2px 6px",
        ...
        <nav className="store-sidebar__nav">
          ...
          <button
            ...
            onClick={() => { localStorage.clear(); window.location.href = "/demo/login"; }}
          >
        ...
        <div className="store-sidebar__version">
          <p className="store-version-label">Demo Version</p>
          <p className="store-version-number">v2.4.1</p>
          <p className="store-version-date">Demo mode — explore freely</p>
        </div>
      </aside>
    </>
  );
...
const DemoHeader: React.FC<HeaderProps> = ({ userName, onMobileMenuOpen }) => {
  ...
  return (
    <header className="store-header">
      ...
      <div className="store-header__actions">
        <span style={{
          background: "rgba(17,181,187,0.12)",
          color: "#11B5BB",
          fontSize: "0.7rem",
          fontWeight: 700,
          padding: "4px 10px",
        ...
        <div className="store-header__user">
          <div className="store-header__avatar" style={{ background: "#F05A28" }}>{initials || "D"}</div>
          <div className="store-header__user-info">
            <p className="store-header__user-name">{userName}</p>
            <p className="store-header__user-role">demo user</p>
          </div>
...
// ─── PIN/QR Modal Component ──────────────────────────────────────────────────
interface PinModalProps {
  pinData: { pin: string; serialNumber: string; deviceName: string; deviceId: string } | null;
  loading: boolean;
  onClose: () => void;
  onConnected: (deviceId: string) => void;
}

const DevicePinModal: React.FC<PinModalProps> = ({ pinData, loading, onClose, onConnected }) => {
  ...
  }, [pinData, isVerified, onConnected]);
  ...
  return (
    <div className="dpin-overlay" onClick={onClose}>
      <div className="dpin-modal" onClick={(e) => e.stopPropagation()}>
        <button className="dpin-close" onClick={onClose}><FaTimes /></button>

        {loading ? (
          <div className="dpin-loading">
            <div className="dpin-spinner" />
            <p>Generating connection PIN...</p>
          </div>
        ) : pinData ? (
          <>
            {isVerified ? (
              <div className="dpin-success-state">
                <div className="dpin-success-icon">
                  <FaCheckCircle size={48} />
                </div>
                <h2>Device Connected!</h2>
                <p>The app has successfully paired with <strong>{pinData.deviceName}</strong>.</p>
                <button className="dpin-done-btn" onClick={onClose}>Done</button>
              </div>
            ) : (
              <>
                <div className="dpin-header">
                  <div className="dpin-header-icon">
                    <FaQrcode size={22} />
                  </div>
                  <h2>Device Connection PIN</h2>
                  <p className="dpin-device-name">{pinData.deviceName}</p>
                  <p className="dpin-serial">Serial: {pinData.serialNumber}</p>
                </div>
                ...
                <div className="dpin-qr-section">
                  ...
                  <p className="dpin-qr-hint">Scan this QR code with the app</p>
                </div>

                <div className="dpin-divider">
                  <span>or enter PIN manually</span>
                </div>

                <div className="dpin-pin-display">
                  {pinData.pin.split("").map((digit, i) => (
                    <div key={i} className="dpin-digit">{digit}</div>
                  ))}
                </div>

                <button className="dpin-copy-btn" onClick={handleCopyPin}>
                  <FaCopy size={12} />
                  {copied ? "Copied!" : "Copy PIN"}
                </button>

                <div className="dpin-footer-note">
                  <p>⏱ This PIN expires in <strong>24 hours</strong></p>
                  <p>Open the app → Scan QR or Enter PIN to connect</p>
                </div>
              </>
            )}
          </>
...
const DemoDashboard: React.FC<DashboardViewProps> = ({ userName, devices, onNavigate }) => {
  const [stats, setStats] = useState({ playlists: 0, announcements: 0 });
  const [loadingStats, setLoadingStats] = useState(true);
  const [showPinModal, setShowPinModal] = useState(false);
  const [pinLoading, setPinLoading] = useState(false);
  const [pinData, setPinData] = useState<{ pin: string; serialNumber: string; deviceName: string; deviceId: string } | null>(null);
  ...
  const handleConnect = async (deviceId: string) => {
    ...
    try {
      const res = await fetch("/api/device-pin/generate", {
        method: "POST",
      ...
      const data = await res.json();
      if (data.success) {
        setPinData({ ...data.data, deviceId });
      } else {
        alert(data.message || "Failed to generate PIN");
        setShowPinModal(false);
      }
    } catch {
      alert("Error generating connection PIN");
      setShowPinModal(false);
    } finally {
      setPinLoading(false);
    }
  ...
  const handleDeviceConnected = (deviceId: string) => {
    if (!connectedDeviceIds.includes(deviceId)) {
      setConnectedDeviceIds([...connectedDeviceIds, deviceId]);
    }
  };
    ...
    <div className="store-dashboard-view">
      ...
      {showPinModal && (
        <DevicePinModal
          pinData={pinData}
          loading={pinLoading}
          onClose={() => { setShowPinModal(false); setPinData(null); }}
          onConnected={handleDeviceConnected}
        />
      )}

      {/* Welcome Banner */}
      <div className="store-welcome-banner" style={{ background: "linear-gradient(135deg, #1a1a2e 0%, #16213e 40%, #0f3460 100%)" }}>
        <div className="store-welcome-banner__content">
          <h1 className="store-welcome-banner__title">Welcome to Demo, {userName} !</h1>
          <p className="store-welcome-banner__subtitle">
            Explore playlists, announcements, and device management. Your demo device is ready to use.
          </p>
        </div>
      ...
      <div className="store-bottom-row">
        ...
        <div className="store-attention-section">
          <div className="store-attention-section__header">
            ...
            <h2 className="store-attention-section__title">Your Demo Device</h2>
          </div>
          ...
          {devices.length === 0 ? (
            ...
            <div className="store-offline-devices">
              {devices.map((device) => {
                ...
                return (
                  <div key={device.id} className="store-device-card" style={{ borderLeftColor: isConnected ? '#11B5BB' : (isOnline ? '#16A34A' : '#F05A28') }}>
                    <div className="store-device-card__content">
                      <div className="store-device-card__icon-wrap">
                        <MdComputer size={24} className="store-device-card__device-icon" style={{ color: isConnected ? '#11B5BB' : (isOnline ? '#16A34A' : '#F05A28') }} />
                      </div>
                    ...
                    <div className="store-device-card__badge" style={{ background: isConnected ? '#EAF6F8' : (isOnline ? '#DCFCE7' : '#FFF2F2'), color: isConnected ? '#11B5BB' : (isOnline ? '#166534' : '#DC2626') }}>
                      <span>{isConnected ? 'connected' : (device.status || 'active')}</span>
                    </div>
                  </div>
                );
              })}
...
// We import these components dynamically to avoid circular dependency issues
// with ViewKey type. We wrap them to pass down the correct ViewKey.
...
export default function DemoDashboardPage() {
  ...
  useEffect(() => {
    ...
    // Only allow demo_store users
    if (userRole !== "demo_store") {
      router.push("/demo/login");
      return;
    }
      ...
      try {
        ...
        if (data?.success && Array.isArray(data.data)) {
          const mapped: OfflineDevice[] = data.data.map((a: any) => ({
            id: a.deviceId?._id || a._id || String(Math.random()),
            name: a.deviceId?.name || "Unknown Device",
            type: a.deviceId?.typeId?.name || "Audio Device",
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case "mediaManagement":
        return <MediaManagementOverview onNavigate={handleNavigate as any} />;
      ...
      case "profile":
        return <ProfileView onNavigate={handleNavigate as any} />;
      default:
        return (
  ...
  return (
    <>
      <style>{`
        ...
        .store-logo-subtitle { font-size: 0.75rem; color: #8CABB3; font-weight: 400; margin-top: 1px; display: flex; align-items: center; }
        ...
        /* ── Connect Button ── */
        .store-device-card__connect-btn {
          background: linear-gradient(135deg, #11B5BB, #0E9C9F);
          color: #fff;
          border: none;
          padding: 8px 18px;
          border-radius: 8px;
          ...
          box-shadow: 0 2px 8px rgba(17,181,187,0.25);
          font-family: inherit;
        }
        .store-device-card__connect-btn:hover {
          transform: translateY(-1px);
          box-shadow: 0 4px 14px rgba(17,181,187,0.35);
        }
        .store-device-card__connect-btn--reconnect {
          background: #F8FAFB;
          color: #11B5BB;
          border: 1px solid #11B5BB;
          box-shadow: none;
        }
        .store-device-card__connect-btn--reconnect:hover {
          background: #EAF6F8;
          box-shadow: none;
        }
        ...
        .dpin-overlay {
          ...
          background: rgba(11,40,48,0.75);
          backdrop-filter: blur(6px);
          z-index: 1000;
          display: flex; align-items: center; justify-content: center;
          padding: 20px;
          animation: dpin-fadein 0.2s ease;
        }
        @keyframes dpin-fadein { from { opacity: 0; } to { opacity: 1; } }
        ...
        .dpin-close {
          position: absolute; top: 16px; right: 16px;
          background: #F8FAFB; border: none;
          width: 32px; height: 32px; border-radius: 8px;
          display: flex; align-items: center; justify-content: center;
          color: #A4B6B9; cursor: pointer; transition: all 0.15s;
        }
        .dpin-close:hover { background: #FFF2F2; color: #DC2626; }
        ...
        .dpin-spinner {
          ...
          border-radius: 50%;
          animation: dpin-spin 0.7s linear infinite;
        }
        @keyframes dpin-spin { to { transform: rotate(360deg); } }
        .dpin-loading p { font-size: 0.9rem; color: #64848D; font-weight: 500; }
        ...
        .dpin-header-icon {
          ...
          box-shadow: 0 4px 16px rgba(17,181,187,0.3);
        }
        .dpin-header h2 { font-size: 1.2rem; font-weight: 800; color: #162B30; margin-bottom: 6px; }
        .dpin-device-name { font-size: 0.88rem; color: #445459; font-weight: 600; }
        .dpin-serial { font-size: 0.75rem; color: #A4B6B9; margin-top: 2px; }
        ...
        .dpin-qr-wrap {
          ...
          border: 2px dashed #D6E6E9;
          border-radius: 16px;
        }
        .dpin-qr-img { border-radius: 8px; display: block; }
        .dpin-qr-hint { font-size: 0.78rem; color: #A4B6B9; margin-top: 10px; font-weight: 500; }

        .dpin-divider {
          display: flex; align-items: center; gap: 12px;
          margin: 16px 0;
        }
        .dpin-divider::before, .dpin-divider::after {
          content: ''; flex: 1; height: 1px; background: #EAEFEF;
        }
        .dpin-divider span {
          font-size: 0.75rem; color: #A4B6B9; font-weight: 600;
          white-space: nowrap;
        }
        ...
        .dpin-digit {
          ...
          background: linear-gradient(135deg, #0B2830, #0D4954);
          border-radius: 12px;
          display: flex; align-items: center; justify-content: center;
          font-size: 1.6rem; font-weight: 800; color: #fff;
          letter-spacing: 0;
          box-shadow: 0 4px 12px rgba(11,40,48,0.2);
        }

        .dpin-copy-btn {
          width: 100%; padding: 12px;
          background: #F8FAFB; border: 1px solid #D6E6E9;
          border-radius: 10px;
          font-size: 0.85rem; font-weight: 700;
          color: #162B30; cursor: pointer;
          display: flex; align-items: center; justify-content: center; gap: 8px;
          transition: all 0.15s; font-family: inherit;
          margin-bottom: 16px;
        }
        .dpin-copy-btn:hover { background: #EAF6F8; border-color: #11B5BB; color: #11B5BB; }
        ...
        .dpin-footer-note p {
          font-size: 0.75rem; color: #64848D; line-height: 1.6;
        }
        .dpin-footer-note strong { color: #F05A28; }

        /* ── Success Animation ── */
        .dpin-success-state {
          display: flex; flex-direction: column; align-items: center;
          text-align: center; padding: 24px 0 12px;
          animation: dpin-fadein 0.4s ease;
        }
        .dpin-success-icon {
          color: #16A34A; margin-bottom: 20px;
          animation: dpin-bounce 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);
        }
        @keyframes dpin-bounce {
          0% { transform: scale(0); }
          50% { transform: scale(1.2); }
          100% { transform: scale(1); }
        }
        .dpin-success-state h2 { font-size: 1.4rem; font-weight: 800; color: #162B30; margin-bottom: 12px; }
        .dpin-success-state p { font-size: 0.95rem; color: #64848D; line-height: 1.5; margin-bottom: 32px; }
        .dpin-success-state strong { color: #162B30; }
        ...
        .dpin-done-btn {
          ...
          cursor: pointer; box-shadow: 0 4px 16px rgba(17,181,187,0.3);
      ...
      <div className="store-root">
        ...
        <div className="store-layout-right">
          <DemoHeader userName={userName} onMobileMenuOpen={() => setMobileOpen(true)} />
